...
async def post_ntfct_body(ntfct_title: str, ntfct_text: str, ntfct_date: str):
    query = f"""
...
async def post_ntfct_for_user(ntfct_id: int, uid: int, ntfct_opened: bool):
    query = f"""
...
async def post_ntfct_for_all(list_ntfct_uid: list):
    query = f"""
         insert into notifications_users(ntfct_id, uid, ntfct_opened) 
         values ($1, $2, $3)
       """
