import app.queries.queries_users
...
async def test_get_top_players_by_active_time(init_db):
    ok = await app.queries.queries_users.get_user_list_by_active_time()
...
async def test_get_email_check_exist(init_db):
    check = await app.queries.queries_users.get_email_check_exist('admin@mail.ru')
    assert check is True


@pytest.mark.asyncio
async def test_get_uid_check_exist(init_db):
    check = await app.queries.queries_users.get_uid_check_exist(11)
    assert check is True


@pytest.mark.asyncio
async def test_get_user_password(init_db):
    password = await app.queries.queries_users.get_user_password('TestUser11')
...
async def test_get_user_name_check_exist(init_db):
    check = await app.queries.queries_users.get_user_name_check_exist('TestUser11')
    assert check is True


@pytest.mark.asyncio
async def test_get_uid_list(init_db):
    uid_list = await app.queries.queries_users.get_uid_list()
    assert uid_list == [{'uid': 11}, {'uid': 14}]


@pytest.mark.asyncio
async def test_get_user_by_uid_check(init_db):
    check = await app.queries.queries_users.get_user_by_uid_check(11)
    assert check is True


@pytest.mark.asyncio
async def test_get_uid_by_user_name(init_db):
    check = await app.queries.queries_users.get_uid_by_user_name('TestUser11')
    assert check == 11


@pytest.mark.asyncio
async def test_get_user_name_by_uid(init_db):
    check = await app.queries.queries_users.get_user_name_by_uid(11)
    assert check == 'TestUser11'


@pytest.mark.asyncio
async def test_get_user_email_by_uid(init_db):
    check = await app.queries.queries_users.get_user_email_by_uid(11)
    assert check == 'roki218@mail.ru'


@pytest.mark.asyncio
async def test_get_user_info_by_uid(init_db):
    check = await app.queries.queries_users.get_user_info_by_uid(11)
